...
def plot_room_temperature(save_fig, res_list):
    ...
    for r in range(num_room):
        ...
        plt.tight_layout()
        if save_fig == 1:
            plt.savefig(f'img/model_temp_comparison_room_{r+1}.jpg', dpi=200)
        else:
            plt.show()
...
def output_res_table(res_list):
    ...
    agg_res.to_csv('results/energy_human_comfort.csv',index=False)

if __name__ == '__main__':
    num_room = 2
    colors = sns.color_palette("tab10")
    data_files = ['results/optimal_solution.csv', 'results/on_off_solution.csv',
