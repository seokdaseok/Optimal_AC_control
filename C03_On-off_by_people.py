...
def on_off_policy(T_room,T_out, target_temp, num_people, room_id):
    Q_AC_r = 0
    if num_people >= Y_hat:
        if T_room > target_temp + on_off_tol:
            if T_out > target_temp:
                Q_AC_r = -A02_optimization_model.cooling_per_5min * alpha[room_id]
        if T_room < target_temp - on_off_tol:
            if T_out < target_temp:
                Q_AC_r = A02_optimization_model.heating_per_5min * alpha[room_id]
...
for t in range(control_period-1):
    ...
    if t == 0:
        ...
        for r in range(num_rooms):
            Q_Ex_r = (T_Out[0] - Rooms_obj[r].T) / A02_optimization_model.R[r] * A02_optimization_model.interval_length
            Q_AC_r = on_off_policy(Rooms_obj[r].T, T_Out[0], A02_optimization_model.Target_temp[r], y[(0,r)], r)
            if Q_AC_r >= 0:
...
res_df.to_csv('results/on_off_with_people_solution.csv',index=False)
