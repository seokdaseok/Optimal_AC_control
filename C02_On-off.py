...
def on_off_policy(T_room, T_out, target_temp, room_id):
    Q_AC_r = 0
...
for t in range(control_period-1):
    ...
    if t == 0:
        ...
        for r in range(num_rooms):
            ...
            Q_AC_r = on_off_policy(Rooms_obj[r].T,T_Out[0], A02_optimization_model.Target_temp[r], r )
            if Q_AC_r >= 0:
...
res_df.to_csv('results/on_off_solution.csv',index=False)
