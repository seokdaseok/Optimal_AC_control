...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.Instructions.insert("0.0", "Hi! This is Frank and Seok's AC Optimization Model designed for Multi-Room Purposes. It is an optimization model that utilizes machine learning principles to predict future temperature based on physical thermal modeling. The following is how to operate our model using this interface: \n\n" +
    ...
    def plot_control_result(self, res_df, heating, cooling, r):
        ...
        idx = 0
        for res in res_list:
            temp = self.ax1.plot(x, res.loc[res['room'] == r, 'T_room'], lw = 2, color=self.colors[idx],  label=model_name[idx]) #marker = 's',
            lns1 += temp
            idx += 1
        ...
        if cooling['t']:
            ...
            labs += [cool.get_label()]
        if heating['t']:
            heat = self.ax1.scatter(heating['t'], [T_max * 1.01] * len(heating['t']), s=50, c=heating['Q'], cmap = 'Reds', alpha=0.2, label='Heating')
            labs += [heat.get_label()]
