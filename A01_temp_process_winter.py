...
data = pd.read_csv('data/temperature_final_winter.csv')

data['Air Temperature(degree Celsius)'] = 0.5 * (data.iloc[:,2] + data.iloc[:,3])
data = data.groupby(['Date time'])['Air Temperature(degree Celsius)'].mean().reset_index()
data['time_str'] = data['Date time'].astype('str')
data['temperature'] = data['Air Temperature(degree Celsius)']
...
final_data.to_csv('data/temperature_final_5_min_winter.csv',index=False)
# a=1
